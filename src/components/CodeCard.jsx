...
const CodeCard = ({ title, description, tags, path }) => {
  ...
  return (
    <div className="my-4 rounded-2xl mx-8 bg-secondary text-white flex flex-col justify-center items-center">
      <p className="w-full h-24 text-3xl text-center flex items-center justify-center border-b-2 border-slate-300 pb-4">
        {title}
      </p>
      <div className="w-full flex flex-col items-center">
        ...
        <div className="flex flex-wrap sm:h-36  lg:h-16 sm:flex-col lg:flex-row justify-center items-center">
          {tags.map((tag) => (
            <div
              key={tag}
              className="bg-[#2A303C] text-white w-24 text-center text-sm rounded-full px-2 m-2"
            >
              {tag}
            </div>
          ))}
        ...
        <Link
          to={`/playground/${path}`}
          className="shadow-2xl text-center m-2 p-2 rounded-full w-48 lg:w-64 text-white bg-indigo-500"
        >
          {' '}
          View{' '}
        </Link>
